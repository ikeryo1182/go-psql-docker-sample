...
//go:cgo_import_dynamic libc_Account Account "libc.so"
...
//go:cgo_import_dynamic libc_fAccountat fAccountat "libc.so"
...
//go:linkname procAccount libc_Account
...
//go:linkname procFAccountat libc_fAccountat
...
func Account(path string, mode uint32) (err error) {
	...
	_, _, e1 := sysvicall6(uintptr(unsafe.Pointer(&procAccount)), 2, uintptr(unsafe.Pointer(_p0)), uintptr(mode), 0, 0, 0, 0)
	if e1 != 0 {
		err = e1
	}
	return
...
func FAccountat(dirfd int, path string, mode uint32, flags int) (err error) {
	...
	_, _, e1 := sysvicall6(uintptr(unsafe.Pointer(&procFAccountat)), 4, uintptr(dirfd), uintptr(unsafe.Pointer(_p0)), uintptr(mode), uintptr(flags), 0, 0)
	if e1 != 0 {
		err = e1
	}
	return
